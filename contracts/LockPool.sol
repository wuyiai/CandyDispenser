...
contract LockPool {
    ...
    function withdraw(address account, uint256 amount) external onlyRewardPool {
        ...
        require(withdrawEntities[account].amount >= amount, "Applied amount is smaller than withdraw amount!");
        require(block.timestamp >= withdrawEntities[account].time, "It's not time to withdraw");
    ...
    function withdrawTime(address account) external view onlyRewardPool returns (uint256) {
        return withdrawEntities[account].time;
    }
}
